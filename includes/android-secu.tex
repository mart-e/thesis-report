...
Paradoxically, the users tend to store more and more personal information on their smartphones and are not aware of the security issues of such devices.
...
It is recommended to disable the debug mode when not required and 
...
When a security flaw is discovered and a patch published, only a very small percentage of users benefit from the patch through an update in the months following the flaw discovered.
...
Due to its ability to install other applications, the Kaspersky Lab's analysts suppose it could have been monetised in the future to be used as a botnet for example.\\
