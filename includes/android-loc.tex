...
$6 (1)$ means there are six surround GSM cell towers detected but only one is valid\footnote{Some cell towers are detected but displayed as using a Cell ID and LAC of -1, these are considered as invalid and ignored. These invalid cells often have a very weak signal strength.}.
...
% A \texttt{Location} is characterized by the following parameters that can be used to determine if a location is or not relevant by an application:
% \begin{itemize}
% \item Longitude
% \item Latitude
% \item Altitude
